...
(define (neighbors node edges)
  (let*
      ([node-and-neighbors (filter
                            (λ (x) (= node (first x)))
                            edges)]
    ...
    (cond
      [(zero? node-and-neighbors-length) '()]
...
(define (longest-el lst) ;;lst must be a list of lists; this returns the longest element of lst
  (cond
    ...
    [(> (length (longest-el (rest lst))) (length (first lst)))
     (longest-el (rest lst))] ;;TODO: recursive call to longest-el is done twice, fix it with let
...
(define (max-cycle edges node)
  (cond
    [(empty? (neighbors node edges)) #F] ;;if no such cycle exists, return #F
...
(define e3 '((1) (1 2) (1 2 3) (pur ple lam bor ghi ni) ()))
